...
<div class="container-fluid">
  <div class="panel panel-default">
    <div class="panel-heading">
      <h3 class="panel-title pull-left">Danh sách đầu tư</h3>
            ...
            <td><?php echo $value['branch_bank'] ?></td> -->
             <td><?php echo date("d/m/Y", strtotime($value['date_added'])); ?></td>
     				...
     				<?php if ($value['status'] == 0) {
                       echo '<td><span class="label label-default">Chờ xử lý</span><a href="index.php?route=pd/pd/showinvoice&transactionId='.$value['pd_number'].'&token='.$token.'"> <i class="fa fa-link"></i>
                              </a></td>';
                    } ?>
                    <?php if ($value['status'] == 1) {
                       echo '<td><span class="label label-info">Đang hoạt động</span></td>';
                    } ?>
                    <?php if ($value['status'] == 2) {
                       echo '<td><span class="label label-success">Kết thúc</span></td>';
                    } ?>
                ...
                <?php $sum = $sum + $value['filled'];  ?>
                        ...
                        <?php $stt++; endforeach; ?>
                        <tr>
                    <td colspan="5" style="text-align: right; color:red; font-size:20px">
                        Tổng:
                    </td>
                    <td colspan="5"> <span style="color:red; font-size:20px"><?php echo number_format($sum); ?> VNĐ </span></td>
                </tr>
