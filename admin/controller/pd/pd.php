...
class ControllerPdPd extends Controller {
	...
	public function showinvoice()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			if ($_POST['status']='ok') {
				$termInvest = $this->model_pd_pd->getpd($pd_number);
				$this -> model_pd_pd -> updateBillPD($pd_number);
				...
				if ($customer['p_node'] && $customer['pnode'] <> 0) {
					...
					if ($pNode['level'] >= 2) {
						...
						$system_decsription = 'Thưởng giới thiệu 10% từ thành viên'.$username['username'].' đầu tư gói'. number_format($termInvest['filled']).' VNĐ';
		...
		$invoice = $this->model_pd_pd->getpd($pd_number);
		...
		$this->response->setOutput($this->load->view('pd/pd-confirm.tpl', $data));
	}
