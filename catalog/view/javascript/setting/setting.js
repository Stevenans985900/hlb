...
*/
              ...
              $.ajax({
                  ...
                  success: function(result) {
                      ...
                      if (result.ok === 1)
                      {
                        ...
                        alertify.success('An email has been sent to your email address. Please check the mail to get back the transaction password !');
                      }
    ...
    $('#changePasswdTransaction').on('submit', function() {
        ...
        if (!$('#TranoldPassword').existsWithValue()) {
            ...
            $('#TranoldPassword').focus();
            $('#TranoldPassword').attr('placeholder','Vui lòng nhập mật khẩu cũ');
        ...
        if (!$('#Tranpassword_New').existsWithValue()) {
            ...
            $('#Tranpassword_New').focus();
            $('#Tranpassword_New').attr('placeholder','Vui lòng nhập mật khẩu mới');
        ...
        if($('#Tranpassword_New').val() !== $('#TranConfirmPassword').val()){
            ...
            $('#TranConfirmPassword').focus();
            $('#TranConfirmPassword').attr('placeholder','Mật khẩu cũ và mới không khớp');
        ...
        if (!checkPasswdTran) {
            ...
            $('#TranoldPassword').focus();
            $('#TranoldPassword').val('');
            $('#TranoldPassword').attr('placeholder','Wrong transaction password, please try again');
    ...
    $('#frmChangePassword').on('submit', function() {
        ...
        if (!$('#Password').existsWithValue()) {
            ...
            $('#Password').focus();
            $('#Password').attr('placeholder','Vui lòng nhập mật khẩu mới');
        ...
        if($('#Password').val() !== $('#ConfirmPassword').val()){
            ...
            $('#ConfirmPassword').attr('placeholder','Mật khẩu cũ và mới không khớp');
        ...
        if(!_checkOldPasswd){
            ...
            $('#OldPassword').focus();
            $('#OldPassword').val('');
            $('#OldPassword').attr('placeholder','Sai mật khẩu cũ, Vui lòng thử lại');
    ...
    $('#updateProfile').on('submit', function(){
        ...
        $(this).ajaxSubmit({
            ...
            beforeSubmit : function(arr, $form, options) {
                ...
                if ($('#Phone').val() == ""){
                    $('#Phone').focus();
                    $('#Phone').attr('placeholder','Please enter the phone number');
                    $('#Phone').css({'border':'1px solid red'});
                    return false;
                }
                if ($('#full_name').val() == ""){
                    $('#full_name').focus();
                    $('#full_name').attr('placeholder','Please enter the full name');
                    $('#full_name').css({'border':'1px solid red'});
                    return false;
                }
                if ($('#address').val() == ""){
                    $('#address').focus();
                    $('#address').attr('placeholder','Please enter the address');
                    $('#address').css({'border':'1px solid red'});
                    return false;
                }
                if ($('#password_transaction').val() == ""){
                    $('#password_transaction').focus();
                    $('#password_transaction').attr('placeholder','Please enter the Password Transaction');
            ...
            success : function(result){
                ...
                if (result.complete == 1){
                    alertify.set('notifier','delay', 30000);
                  alertify.set('notifier','position', 'top-right');
                ...
                if (result.password == -1){
                    ...
                    $('#password_transaction').attr('placeholder','Password transaction wrong !')
                    return false;
                }
            }
        });
        ...
        $(this).ajaxSubmit({
            ...
            beforeSubmit : function(arr, $form, options) {
               ...
               /* if ($('#Perfect_Money').val() == ""){
                ...
                if ($('#Password2').val() == ""){
                    $('#Password2').focus();
                    $('#Password2').attr('placeholder','Please enter the Password Transaction');
    ...
    $('#updateIPaddress').on('submit', function(){

        $(this).ajaxSubmit({
            type : 'POST',
            beforeSubmit : function(arr, $form, options) {
                if ($('#ip_address_ip').val() == ""){
                    $('#ip_address_ip').focus();
                    $('#ip_address_ip').css({'border':'1px solid red'});
                    return false;
                }
            },
            success : function(result){
                ...
                if (result.complete == 1){
                    alertify.set('notifier','delay', 3000);
                    alertify.set('notifier','position', 'top-right');
                    alertify.success('Update Restrict API Access successfull !!!');
    ...
    $('#updateLoginAlerts').on('submit', function(){
        ...
        $(this).ajaxSubmit({
            ...
            success : function(result){
                ...
                if (result.complete == 1){
                    alertify.set('notifier','delay', 3000);
                    alertify.set('notifier','position', 'top-right');
                    alertify.success('Update Login Alerts successfull !!!');
                    setTimeout(function(){ location.reload(true); ; }, 1000);
                }
            }
        });
    ...
    $('#updateauthenticator').on('submit', function(){

        $(this).ajaxSubmit({
            type : 'POST',
            beforeSubmit : function(arr, $form, options) {
               if ($('#ip_authenticator').val() == ""){
                    $('#ip_authenticator').focus();
                    $('#ip_authenticator').css({'border':'1px solid red'});
                    return false;
                }
            },
            success : function(result){
    ...
    $('#updateimg_profile').on('submit', function(){
