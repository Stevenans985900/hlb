...
class ModelAccountAuto extends Model {
	...
	public function update_ml_noactive_in12hour(){

		$query = $this -> db -> query("
			UPDATE ". DB_PREFIX . "customer_ml SET p_binary = 0 WHERE DATE_ADD(date_added, INTERVAL 12 HOUR) <= NOW() AND level = 1
	...
	public function getCustomerALL_ML(){
		$query = $this -> db -> query("
			SELECT ml.*, c.cycle
			FROM ". DB_PREFIX . "customer_ml ml LEFT JOIN ". DB_PREFIX . "customer c ON ml.customer_id = c.customer_id
			WHERE level = 2
		");
		return $query -> rows;
	}
	...
	public function get_check_c_wallet()
	{
		$query = $this -> db -> query("
			SELECT pd.*, c.p_node, c.username 
			FROM  ".DB_PREFIX."customer_provide_donation pd LEFT JOIN ".DB_PREFIX."customer c ON pd.customer_id = c.customer_id
			WHERE pd.status = 1 AND pd.check_c_wallet = 1
		");
		return $query -> rows;
	}
