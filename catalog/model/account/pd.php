...
class ModelAccountPd extends Model {
	...
	// ====================================================
	public function countInvestmentTerm($customer_id){
		$query = $this -> db -> query("
			SELECT COUNT(*) AS number
	...
	public function CreateInvestTerm($amount, $max_profit, $term){
		...
		$this -> db -> query("
			INSERT INTO ". DB_PREFIX . "customer_investment_term SET 
			customer_id = '".$this -> session -> data['customer_id']."',
			date_added = NOW(),
			filled = '".$amount."',
		...
		$pd_number = hexdec( crc32($pd_id.$date_added) );
		$query = $this -> db -> query("
	...
	public function updateBill($pd_number, $image){
		$query = $this -> db -> query("
			UPDATE " . DB_PREFIX . "customer_investment_term SET
				images = '".$image."'
				WHERE pd_number = '".$pd_number."'
	...
	public function updateBillPD($pd_number, $image){
		$query = $this -> db -> query("
			UPDATE " . DB_PREFIX . "customer_provide_donation SET
				images = '".$image."'
				WHERE pd_number = '".$pd_number."'
	...
	public function getInvestTermByCusID($customer_id){
		$query = $this -> db -> query("
			SELECT * 
			FROM ". DB_PREFIX . "customer_investment_term
