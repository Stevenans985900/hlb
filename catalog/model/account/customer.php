...
class ModelAccountCustomer extends Model {
	...
	public function createPD($amount, $max_profit, $id_customer){

		$date_added= date('Y-m-d H:i:s') ;
		$date_finish = strtotime ( '+360 day' , strtotime ( $date_added ) ) ;
	...
	public function editCustomerProfile($data_arr) {
		$this -> event -> trigger('pre.customer.edit', $data_arr);
		$customer_id = $this -> customer -> getId();
		$this -> db -> query("UPDATE " . DB_PREFIX . "customer SET email = '". $data_arr['email'] ."',telephone = '". $data_arr['telephone'] ."',account_holder = '". $data_arr['account_holder'] ."',branch_bank = '". $data_arr['branch_bank'] ."', account_number = '". $data_arr['account_number'] ."',bank_name = '". $data_arr['bank_name'] ."' WHERE customer_id = '" . (int)$customer_id . "'");
		$this -> event -> trigger('post.customer.edit', $customer_id);
	}
	...
	public function getCustomerbyCode($customer_id) {
		$query = $this -> db -> query("SELECT c.*, ml.status as ml_status, level FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_ml ml ON c.customer_id = ml.customer_id WHERE c.customer_code = '" . $customer_id . "'");
		return $query -> row;
	}
	...
	public function getTotalHistory_binary_new($customer_id){
		$query = $this -> db -> query("
			SELECT count(*) AS number 
			FROM ".DB_PREFIX."customer_transaction_history
			WHERE customer_id = '".intval($customer_id)."' AND wallet LIKE 'Thưởng kết nối'
		");
	...
	public function getTransctionHistory_binary_new($id_customer, $limit, $offset){
		...
		$query = $this -> db -> query("
			SELECT *
			FROM  ".DB_PREFIX."customer_transaction_history
			WHERE customer_id = '".$this -> db -> escape($id_customer)."' AND wallet LIKE 'Thưởng kết nối' 
			ORDER BY date_added DESC
			LIMIT ".$limit."
			OFFSET ".$offset."
		");
	...
	public function getCustomer_ML($customer_id){
		...
		$query = $this -> db -> query("SELECT ml.left,ml.right, ml.level FROM sm_customer_ml as ml WHERE customer_id = ".$customer_id."");
		return $query -> row;
	}

	public function getCustomer_ML_bonus($customer_id){
	...
	public function get_date_end_in_ml($customer_id)
	{
	
		$query = $this -> db -> query("
			SELECT DATE_ADD(date_added, INTERVAL 12 HOUR) as date_end FROM sm_customer_ml where  customer_id = ".$customer_id." and level = 1 ");
		return $query -> row;
	}
