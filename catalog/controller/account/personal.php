...
public function checkBinary($p_binary){
	...
	public function add_customer (){
		...
		$customer_binary = $this -> model_account_customer -> getCustomer_ML($customer['customer_id']);
		!$customer_binary || $customer_binary['level'] == 1 && $this -> response -> redirect(HTTPS_SERVER . 'login.html');
	...
	public function checkcmnd($cmnd) {
		if ($cmnd) {
			...
			$json['success'] = intval($this -> model_customize_register -> checkExitCMND($cmnd)) < 1 ? 0 : 1;
			return $json['success'];
		}
	}
	...
	public function register_submit(){
		...
		if ($this->request->server['REQUEST_METHOD'] === 'POST'){
			...
			if (intval($check_p_binary['number']) === 2) {
				...
				$mycustomer = $this -> model_customize_register -> getCustomer_ml($cus_id);

				switch (intval($mycustomer['status'])) {
					case 1:
						$amount = 1000000;
						break;
					case 2:
				...
				$pd = $this -> model_account_customer ->createPD($amount, 0, $cus_id);
