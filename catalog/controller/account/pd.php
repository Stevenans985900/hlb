...
class ControllerAccountPd extends Controller {
    ...
    public function TermInvestment()
    {
        function myCheckLoign($self) {
            return $self -> customer -> isLogged() ? true : false;
        };
        ...
        $data['TermInvestment'] = $this -> model_account_pd -> getInvestTermByCusID($this -> session -> data['customer_id']);
    
        $server = $this -> request -> server['HTTPS'] ? $server = $this -> config -> get('config_ssl') : $server = $this -> config -> get('config_url');
        $data['base'] = $server;
        $data['self'] = $this;

        if (file_exists(DIR_TEMPLATE . $this -> config -> get('config_template') . '/template/account/term-invest.tpl')) {
            $this -> response -> setOutput($this -> load -> view($this -> config -> get('config_template') . '/template/account/term-invest.tpl', $data));
        } else {
            $this -> response -> setOutput($this -> load -> view('default/template/account/term-invest.tpl', $data));
        }
    }
    ...
    public function TermInvestmentSubmit()
    {
        $this -> load -> model('account/pd');
        // $json=array();
        if ($this -> customer -> isLogged() && $this -> request -> post['amount'] && $this -> request -> post['term']) {
            $amount = $this -> request -> post['amount'];
            $term = $this -> request -> post['term'];
            $countInvest = $this -> model_account_pd -> countInvestmentTerm($this -> session -> data['customer_id']);
            
            if (intval($countInvest['number']) >= 3) {
                $json['success'] = -1;
            }else{
                $max_profit = 0;
                $InvestTerm = $this -> model_account_pd -> CreateInvestTerm($amount ,$max_profit, $term);
                $number = $InvestTerm['pd_number'];
                $json['link'] =  HTTPS_SERVER . 'invoice&transactionId='.$number;
                $json['success'] = 1;
            }
	 ...
	 public function show_invoice()
    {
        ...
        !array_key_exists('transactionId', $this->request->get) && die();
        $pd_number = $this->request->get['transactionId'];
     
        $invoice = $this->model_account_pd->getInvestTerm($pd_number, $this->session->data['customer_id']);
        ...
        $data['link'] =  HTTPS_SERVER . 'invoice&transactionId='.$pd_number;
        $data['self'] = $this;
    ...
    public function uploadBill(){
        ...
        if ($this -> customer -> isLogged() && $this -> request -> files && $this -> request -> post['number'])
        {
        
            $this -> load -> model ('account/customer');
                $file = $this -> bill($this -> request -> files, $this -> request -> post['number']);
                $json['link'] =  HTTPS_SERVER . 'term-investment.html';
                $json['complete'] = 1;
                $this -> response -> setOutput(json_encode($json));
            
        }
    }
    public function uploadBillPD(){

        if ($this -> customer -> isLogged() && $this -> request -> files && $this -> request -> post['number'])
        {
        
            $this -> load -> model ('account/customer');
                $file = $this -> billpd($this -> request -> files, $this -> request -> post['number']);
                $json['link'] =  HTTPS_SERVER . 'investing.html';
                $json['complete'] = 1;
                $this -> response -> setOutput(json_encode($json));
            
        }
    } 
    public function billpd($file,$pd_number){
        $this->load->model('account/pd');
        ...
        $this -> model_account_pd -> updateBillPD($pd_number,$linkImage);

        
    }
    public function bill($file,$pd_number){
        $this->load->model('account/pd');
        ...
        $this -> model_account_pd -> updateBill($pd_number,$linkImage);
